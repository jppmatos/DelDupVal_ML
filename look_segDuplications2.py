...
def closest_to_CNV(df_raw_regions,dict_Centro):
    ...
    for index, row in df_raw_regions.iterrows():
        ...
        sub_df_region_B = row[['regionB_stat','regionB_end','chr_B']] 
            #
        regionA_data = assigned_region_data(sub_df_region_A,dict_Centro)#
        regionB_data = assigned_region_data_B(sub_df_region_B,dict_Centro)# #B!
...
def assigned_region_data(sub_df_CNV,dict_Centro):
    ...
    sub_df_Centro.columns=['chromStart','chromEnd','region_name','region_family','region_class','region_strand','r_chrom']
    ...
    for i in range(len(sub_df_Centro)):
        ...
        if get_Overlap(a,b) == True:
            ...
            posA = (sub_df_CNV.iloc[0] - sub_df_Centro['chromEnd'].iloc[i]).item()
            #posB = (sub_df_Centro['chromStart'].iloc[i] - sub_df_CNV.iloc[1]).item()
    ...
    return add_list
            #dict_items[CNV_id]=add_list
            
        #print(dict_items)#DEBUG
...
def assigned_region_data_B(sub_df_CNV,dict_Centro): #check it if it is OKKAY!!!!
    """Finds region name, family and class - just segmental duplications.
        sub_df_CNV is a region!!
    """
    selected_chrom = sub_df_CNV.iloc[-1]
    #selected_chrom = sub_df_CNV['chr'].iloc[0]
...
df_selec_regions.to_csv(outPATH + 'info_segmentalDups_matched_test.csv', sep = ';' ,index_label=False)
