...
def get_segmentalDUPS(region_stat,region_end,chrom,dict_Centro):#,df_genes):
    ...
    #
    #print(sub_df_segDUPS.head(20))#DEBUG
    #Just segmental duplications:
    sub_df_segDUPS = sub_df_segDUPS.loc[sub_df_segDUPS['region_class'] == 'Dup'] #FOR ONLY Segmental Duplications !!!
    #print(sub_df_segDUPS.shape)
    #---
...
# In[16]:
#To a dataframe:
#print(Genes_Data[0])#DEBUG
df_Genes = pd.DataFrame(Genes_Data, columns=['CNV_ID','Cluster_ID',
                                       'A_cov_segDup','B_cov_segDup'])#,'Name_A','Family_A','Class_A','Name_B','Family_B','Class_B'])

#Save to a csv:
output_path = argv[2]
df_Genes.to_csv(output_path + 'segDuplications_coverage_regionsAB.csv', sep=';',index_label=False)
